...
/** Abstract ice blocks that drift in for the final "Dingin" stage. */
export function IceCubes({ refs }: { refs: SceneRefs }) {
  const group = useRef<THREE.Group>(null);

  const cubes = useMemo(() => {
    const rand = createRandom(SEEDS.ice);
    return Array.from({ length: 7 }, (_, i) => ({
      position: new THREE.Vector3(
        (rand() - 0.5) * 4.2,
        (rand() - 0.5) * 3,
        THREE.MathUtils.lerp(-1.6, 1.1, rand()),
      ),
      ...
      scale: THREE.MathUtils.lerp(0.14, 0.28, rand()),
      drift: 0.12 + i * 0.03,
    }));
  }, []);
  ...
  useFrame((state, delta) => {
    ...
    if (!g) return;
    const stage = toStage(refs.progress.current);
    const presence = band(stage, 2.35, 3.5, 0.45);
    ...
    g.children.forEach((child, i) => {
      ...
      const t = state.clock.elapsedTime;
      mesh.rotation.x = cube.rotation.x + t * cube.drift * 0.4;
      mesh.rotation.y = cube.rotation.y + t * cube.drift * 0.55;
      mesh.position.y = cube.position.y + Math.sin(t * cube.drift + i) * 0.16;
      mesh.scale.setScalar(cube.scale * (0.6 + presence * 0.4));
    });
